...
Full requirements are available in the `requirements.txt` file.
...
What you supply under a given relay in this chat should be in accordance with how the relay's class works. For example, a discord relay should come with id's (integers) of channels intended to be forwarded information to. You can have as many relays listed in this file under any given chat pool as long as it's defined in `relays.yaml` and its `active` attribute set to `true`.## Dependencies
You need the following packages installed:
